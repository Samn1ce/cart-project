...
function toggleCart(food) {
  ...
  if (food.inCart) {
    food.quantity = 1; // Set quantity to 1 when added to cart
  }
}
...
function addToCart(item) {
  // Step 1: Retrieve the existing cart from localStorage, or create an empty array if not found
  let currentCart = JSON.parse(localStorage.getItem('cart')) || [];

  // Step 2: Add the new item to the cart array
  currentCart.push(item);

  // Step 3: Update the cartData array (Vue ref)
  cartData.value = currentCart;
